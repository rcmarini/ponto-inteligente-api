...
public class LancamentoControllerTest {
	...
	@Test
	@WithMockUser
	public void testeCadastrarLancamento() throws Exception {
	...
	@Test
	@WithMockUser
	public void testeCadastrarLancamentoFuncionarioIdInvalido() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.post(URL_BASE).content(this.obterJsonRequisicaoPost())
				...
				.andExpect(jsonPath("$.errors").value("Funcionário não encontrado, ID inexistente: " + ID_FUNCIONARIO))
				.andExpect(jsonPath("$.data").isEmpty());
	}

	@Test
	@WithMockUser(username = "admin@teste.com", roles = { "ADMIN" })
	public void testeRemoverLancamento() throws Exception {
		BDDMockito.given(this.lancamentoService.buscarPorId(Mockito.anyLong()))
				.willReturn(Optional.of(new Lancamento()));

		mvc.perform(MockMvcRequestBuilders.delete(URL_BASE + ID_LANCAMENTO).contentType(MediaType.APPLICATION_JSON)
	...
	@Test
	@WithMockUser
	public void testeRemoverLancamentoAcessoNegado() throws Exception {
		BDDMockito.given(this.lancamentoService.buscarPorId(Mockito.anyLong()))
				.willReturn(Optional.of(new Lancamento()));
